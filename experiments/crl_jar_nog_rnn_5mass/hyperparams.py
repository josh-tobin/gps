...
MODEL_PATH = './mjc_models/PR2/pr2_jar_nogravity.xml' 

SAMPLE_STRATEGY = 'manual'
MASSES = [0.15, 0.2, 0.3, 0.4, 0.45] 
MASSES = [np.array([m,]) for m in MASSES]
MASS_STD = 0.25
MASS_RANGE = np.array([0.4, 0.4])
GAIN_RANGE = np.array([1.0, 1.0])

EXP_DIR = BASE_DIR + '/../experiments/crl_jar_nog_rnn_5mass/'
BATCH_SIZE = 10
N_EPOCHS = 350
...
elif SAMPLE_STRATEGY == 'manual':
    mass_body_mult = MASSES
    gain_scale = [np.ones(7)]*common['conditions']
