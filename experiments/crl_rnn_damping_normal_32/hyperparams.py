...
from gps.algorithm.cost.cost_utils import RAMP_FINAL_ONLY, RAMP_LINEAR
...
import itertools
from gps.utility.general_utils import find_bodies_in_model

from gps.algorithm.policy_opt.tf_rnn import crl_rnn_network, crl_rnn_large, CRLRNN
...
MASS_STD = 0.25
MASS_RANGE = np.array([1.0, 1.0])
GAIN_RANGE = np.array([1.0, 1.0])
DAMPING_RANGE = np.array([1.0, 1.0])
DAMPING_STD = 0.25
GAIN_STD = 0.0

EXP_DIR = BASE_DIR + '/../experiments/crl_rnn_damping_normal_32/'
BATCH_SIZE = 10
N_EPOCHS = 350
MODEL = CRLRNN(batch_size=BATCH_SIZE, n_steps = 100,
               rnn_output_dim=64, hidden_dim=32, n_feedforward=3,
               n_recurrent=3)
...
MODEL_PATH = './mjc_models/PR2/pr2_1arm.xml' 
...
if SAMPLE_STRATEGY == 'uniform':
    mass_body_mult = [np.random.uniform(low=MASS_RANGE[0], 
                                         high=MASS_RANGE[1], size=num_bodies)
                       for _ in range(common['conditions'])]

    gain_scale = [np.random.uniform(low=GAIN_RANGE[0],
                                    high=GAIN_RANGE[1], size=7)
                  for _ in range(common['conditions'])]
    damping_scale = [np.random.uniform(low=DAMPING_RANGE[0],
                                       high=DAMPING_RANGE[1], size=(7,1))
                      for _ in range(common['conditions'])]
elif SAMPLE_STRATEGY == 'normal':
     
    if GAIN_STD > 0:
        gain_scale = [np.random.normal(loc=1.0, 
                                       scale=GAIN_STD, size=7) 
                        for _ in range(common['conditions'])]
    else:
        gain_scale = [np.ones(7) for _ in range(common['conditions'])]
    mass_body_mult = [np.random.normal(loc=1.0, 
                                       scale=MASS_STD, size=num_bodies) 
                      for _ in range(common['conditions'])]
    damping_scale = [np.random.normal(loc=1.0, scale=DAMPING_STD,
                                      size=(7, 1))
                     for _ in range(common['conditions'])]
elif SAMPLE_STRATEGY == 'all':
    mass_body_mult = [np.ones(num_bodies)]
    mass_body_mult += list(itertools.product(MASS_RANGE, repeat=num_bodies))
    gain_scale = [np.ones(7)]*common['conditions']
...
agent = {
    ...
    'mass_body_idx': np.array(body_idx),
    'mass_body_mult': mass_body_mult,
    'gain_scale': gain_scale,
    'damping_mult': damping_scale,
