...
from gps.utility.general_utils import get_ee_points, find_objects_in_model, \
                                      find_sites_in_model
...
EXP_DIR = BASE_DIR + '/../experiments/crl_swingup_ff/'
MODEL_FILE = './mjc_models/PR2/pr2_swingup.xml' 
...
objects = find_objects_in_model(MODEL_FILE)
print objects
obj_idx = objects.values()
num_obj = len(obj_idx)
print num_obj
sites = find_sites_in_model(MODEL_FILE)
print "sites"
print sites
...
agent = {
    ...
    'dt': 0.05,
    'substeps': 50,
    'ee_point_mode': 'moving_target',
    'ee_points_indices': sites['center_of_ball'],
    'ee_points_tgt_site': sites['ball_target'],
    'ee_points_tgt': np.zeros(3),
    'conditions': common['conditions'],
    'mass_body_idx': np.array(obj_idx),
    'body_color_offset': np.array([1]),
    'mass_body_mult': np.array([1.0] * num_obj),
...
ee_pts_tgt = np.zeros([3,])
print "EE_POINTS_SHAPE"
print EE_POINTS.shape[0]
fk_cost = {
    'type': CostFK,
...
algorithm['cost'] = {
    'type': CostSum,
    'costs': [fk_cost, torque_cost, final_cost],
    'weights': [0.1, 1.0, 1.0],
}
