...
SAMPLE_STRATEGY = 'some'
MASS_STD = 0.25
MASS_RANGE = np.array([0.75, 1.25])
GAIN_RANGE = np.array([1.0, 1.0])

EXP_DIR = BASE_DIR + '/../experiments/crl_rnn_ml_smm12/'
BATCH_SIZE = 10
N_EPOCHS = 350
...
if SAMPLE_STRATEGY == 'uniform':
    ...
    gain_scale = [np.ones(7)]*len(mass_body_mult)
elif SAMPLE_STRATEGY == 'some':
    mass_body_mult = [np.ones(num_bodies)]
    combos = list(itertools.product(MASS_RANGE, repeat=num_bodies))
    indicies = np.random.choice(len(combos), common['conditions'] -1)
    mass_body_mult += [combos[i] for i in range(len(combos)) if i in indicies]
