...
def init_lqr(hyperparams):
    """
    Return initial gains for a time-varying linear Gaussian controller that tries to hold the
    initial position.
    """
    ...
    Vxx_t = np.zeros((dX, dX))  # Vxx = ddV/dXdX. Second derivative of value function.
...
def init_pd(hyperparams):
    ...
    if dU < dQ:
        K = -config['init_stiffness'] * np.tile(
                [np.eye(dU) * Kp, np.zeros(dU, dQ-dU), np.eye(dU) * Kv, np.zeros((dU, dQ-dU))],
                [T, 1, 1]
        )
    else:
        K = -config['init_stiffness'] * np.tile(
