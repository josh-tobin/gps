...
class TrajOptLQRPython(TrajOpt):
    ...
    def update(self, m, algorithm):
        ...
        """
        T, step_mult, prev_eta = algorithm.T, algorithm.cur[m].step_mult, algorithm.cur[m].eta
        traj_info, prev_traj_distr = algorithm.cur[m].traj_info, algorithm.cur[m].traj_distr
        ...
        for itr in range(DGD_MAX_ITER):
            traj_distr, new_eta = self.backward(prev_traj_distr, traj_info, prev_eta, algorithm, m)
            ...
            # Compute KL divergence between previous and new distribution.
            ...
            eta = line_search.bracketing_line_search(kl_div - kl_step*T, new_eta, min_eta)

            # Convergence check - dual variable change when min_eta hit.
            if (abs(prev_eta - eta) < THRESHA and eta == max(min_eta, self._hyperparams['min_eta'])):
                LOGGER.debug("Iteration %i, KL: %f / %f converged (eta limit)", itr, kl_div,
                        kl_step * T)
                break

            # Convergence check - constraint satisfaction, KL not changing much.
            if (itr > 2 and abs(kl_div - prev_kl_div) < THRESHB and kl_div < kl_step*T):
                LOGGER.debug("Iteration %i, KL: %f / %f converged (no change)", itr, kl_div,
                        kl_step * T)
                break

            prev_kl_div = kl_div
            LOGGER.debug('Iteration %i, KL: %f / %f eta: %f -> %f', itr, kl_div, kl_step * T,
                    prev_eta, eta)
            prev_eta = eta
    ...
    def backward(self, prev_traj_distr, traj_info, eta, algorithm, m):
        ...
        Args:
            ...
            traj_info: A TrajectoryInfo object (need dynamics and cost matrices).
            eta: Dual variable.
        ...
        while fail:
            ...
            for t in range(T - 1, -1, -1):
                ...
                traj_distr.pol_covar[t,:,:] = sp.linalg.solve_triangular(U,
                        sp.linalg.solve_triangular(L, np.eye(dU), lower=True))
                traj_distr.chol_pol_covar[t,:,:] = sp.linalg.cholesky(traj_distr.pol_covar[t,:,:])

                # Compute mean terms.
                traj_distr.k[t,:] = -sp.linalg.solve_triangular(U,
                        sp.linalg.solve_triangular(L, Qt[idx_u], lower=True))
                traj_distr.K[t,:,:] = -sp.linalg.solve_triangular(U,
                        sp.linalg.solve_triangular(L, Qtt[idx_u,idx_x], lower=True))
            ...
            if fail:
                ...
                if eta >= 1e16:
                    ...
                    raise ValueError('Failed to find positive definite solution even for very large \
                            eta (check that dynamics and cost are reasonably well conditioned)!')
        return traj_distr, eta
