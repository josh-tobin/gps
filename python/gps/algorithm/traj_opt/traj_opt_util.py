...
def traj_distr_kl(new_mu, new_sigma, new_traj_distr, prev_traj_distr):
    """
    Compute KL divergence between new and the previos trajectory distributions.
    Args:
    ...
    for t in range(T):
        ...
        prc_prev = sp.linalg.solve_triangular(chol_prev,
                sp.linalg.solve_triangular(chol_prev.T, np.eye(dU), lower=True))
        prc_new = sp.linalg.solve_triangular(chol_new,
                sp.linalg.solve_triangular(chol_new.T, np.eye(dU), lower=True))
        ...
        if not self.data or ((abs(self.data['c1']-con) < 1e-8*abs(self.data['c1']+con)) and
            ...
            if (self.data['c1'] <= con and self.data['c2'] > con and
                ...
                if (self.data['c1'] < 0 and self.data['c2'] < 0 and con < 0 and
                        ...
                        abs(self.data['c2']-con) / (max(abs(self.data['c2']),
                            abs(con))*abs(np.log(self.data['e2'])-np.log(eta))) < 1e-2):
            ...
            # First, try to perform a log-space fit. Note that this may no longer be convex.
            lc1 = c1 * e1
            lc2 = c2 * e2
            le1 = np.log(e1)
            le2 = np.log(e2)
            ...
            # Decide whether we want to solve in the original space instead. Use ratio of gradients.
            # Solve in the original if the ratio of gradients is very high in log space (with 50
            # percent probability).
            ...
            if a < 0 and not solve_orig:  # Concave, good to go.
                ...
                if solve_orig:
                    LOGGER.debug('Solving non-log problem due to ratio choice: %f %f',
                            abs(np.log(abs(lc1 / lc2))), abs(np.log(abs(c1 / c2))))
                if not solve_orig:
                    LOGGER.debug('Solving non-log problem due to a > 0: %f %f',
                            abs(np.log(abs(lc1 / lc2))),abs(np.log(abs(c1 / c2))))
