    ...
    def bracketing_line_search(self, con, eta, min_eta):
        ...
        if not self.data or ((abs(self.data['c1']-con) < 1e-8*abs(self.data['c1']+con)) and
            ...
            if (self.data['c1'] <= con and self.data['c2'] > con and
                ...
                mid = 0
                if (self.data['c1'] < 0 and self.data['c2'] < 0 and con < 0 and  # Too little change in all cases.
                        eta <= self.data['e1'] and eta <= self.data['e2'] and  # eta is decreasing.
                        self.data['c1'] != self.data['c2'] and
                        abs(self.data['c2']-con)/(max(abs(self.data['c2']),abs(con))*abs(np.log(self.data['e2'])-np.log(eta))) < 1e-2):
