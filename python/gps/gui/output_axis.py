...
class OutputAxis(object):
    """ Output axis class. """
    def __init__(self, axis, log_filename=None, max_display_size=10,
                 border_on=False, bgcolor='white', bgalpha=0.0,
                 font_family='sans-serif'):
        self._axis = axis
        self._fig = axis.get_figure()
        self._log_filename = log_filename

        self._text_box = self._axis.text(
            0.02, 0.95, '', color='black', fontsize=12, va='top', ha='left',
            transform=self._axis.transAxes, family=font_family
        )
        self._text_arr = []
    ...
    def draw(self):
        ...
        self._axis.set_axis_bgcolor(
            self.cc.to_rgba(self._bgcolor, self._bgalpha)
        )
