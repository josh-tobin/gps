...
class Sample(object):
    ...
    def __init__(self, sample_data):
        ...
        self._X = np.empty((self.dX, self.T))
        self._X.fill(np.nan)
        self._obs = np.empty((self.dO, self.T))
    ...
    def set(self, sensor_name, sensor_data, t=None):
        ...
        if t is None:
            ...
            if sensor_name not in self._data:
                self._data[sensor_name] = np.empty((sensor_data.shape[0], self.T))
    ...
    def get_X(self, t=None):
        ...
        if np.any(np.isnan(X)):
            for data_type in self._data:
                self.sample_data.pack_data_x(X, self._data[data_type], data_types=[data_type], t=t)
        return X
    ...
    def get_obs(self, t=None):
        ...
        if np.any(np.isnan(obs)):
            ...
            for data_type in self._data:
                self.sample_data.pack_data_obs(obs, self._data[data_type], data_types=[data_type], t=t)
        return obs
